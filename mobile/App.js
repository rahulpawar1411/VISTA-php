...
//
// API URL:
//   - Local: getLocalApiUrl() -> http://LAN-IP:5080 (PHP backend)
...
/** PRODUCTION ke liye yeh API use karo (Hostinger PHP). No trailing slash, no /api. */
...
const FALLBACK_LOCAL_IP = '192.168.137.129';
    ...
    const restoreSession = async () => {
      ...
      try {
        ...
        if (storedApiUrl && storedApiUrl.trim()) {
          ...
          // Migrate old Node / wrong hosts -> current Hostinger PHP API
          if (/railway\.app|onrender\.com|backend-php/i.test(nextApi)) {
            nextApi = PRODUCTION_API_URL;
            console.log('[api] migrated legacy URL -> Hostinger PHP');
          }
          // Keep saved Hostinger URL on current production host
          if (
            /hostingersite\.com/i.test(nextApi) &&
            nextApi.replace(/\/$/, '') !== PRODUCTION_API_URL
          ) {
            nextApi = PRODUCTION_API_URL;
            console.log('[api] refreshed Hostinger API URL ->', nextApi);
          }
