...
function buildImageCandidates(raw, apiUrl, folderHint = 'daily_temp_monitor_images') {
  ...
  resolveLogImageUrlCandidates(raw, apiUrl, folderHint).forEach(push);

  const prod = String(PRODUCTION_API_URL || '').replace(/\/$/, '');
  const current = String(apiUrl || '').replace(/\/$/, '').replace(/\/api$/i, '');
  if (prod && prod !== current) {
    resolveLogImageUrlCandidates(raw, prod, folderHint).forEach(push);
  }
...
export default function CustomerScreen({ user, token, apiUrl, onLogout, onUserUpdate }) {
  ...
  const resolveDockPhotoUri = useCallback((path, hint) => {
    ...
    resolveLogImageUrlCandidates(path, apiUrl, hint).forEach(push);
    resolveLogImageUrlCandidates(path, PRODUCTION_API_URL, hint).forEach(push);
    return urls;
  }, [apiUrl]);
